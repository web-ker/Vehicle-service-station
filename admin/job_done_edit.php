...
$sql="select * from job_done where jobdone_id='$jobdone_id'";
$res=mysql_query($sql);
$row=mysql_fetch_array($res); 
?>
<form name="formID" id="formID" method="post" action="job_done_update.php">
 <input type="hidden" name="jobdone_id" value="<?php echo $row['jobdone_id'];?>">
  <table width="270" border="0">
    <tr>
      <td colspan="2">Job Done </td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  {
	  ?>
        <option value="<?php echo $row2['jobcard_id'];?>"<?php if($row2['jobcard_id']==$row['jobcard_id']){ ?> selected <?php }?> ><?php echo $row2['jobcard_date'];?></option>
        <?php
	  }
	  ?>
      </select></td>
